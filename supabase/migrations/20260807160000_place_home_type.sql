-- Home type (apartment / condo / townhome / single family) for place form + filters

alter table app_next_chapter_v1.places
  add column if not exists home_type text
    check (
      home_type is null
      or home_type in ('apartment', 'condo', 'single_family', 'townhome')
    );

create index if not exists idx_next_chapter_places_home_type
  on app_next_chapter_v1.places (home_type)
