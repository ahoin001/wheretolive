...
export function FloatingTierChips({
  ...
  return (
    <div
      ...
      className="flex w-full min-w-0 basis-full gap-1 border-b border-line/70 pb-2"
    >
      {TIERS.map((tier) => {
        ...
        return (
          <button
            ...
            className={`min-h-9 min-w-0 flex-1 rounded-xl border px-1 text-[11px] font-bold leading-tight ${
              active
                ? meta.chipActive
                : `bg-panel ${meta.chip} hover:border-sea/50`
            }`}
          >
            <span className="block truncate">{meta.short}</span>
            <span className="block tabular-nums opacity-80">{count}</span>
          </button>
        )
      })}
    </div>
...
export function SelectionDock({
  ...
  return (
    ...
    >
      {tierNav ? <FloatingTierChips {...tierNav} /> : null}
      {selectMode ? (
        <>
...
export function SelectionDockSpacer({
  ...
  return (
    <div
      className={tall ? 'h-28 shrink-0 md:h-16' : 'h-20 shrink-0 md:h-16'}
      aria-hidden
    />
  )
}
